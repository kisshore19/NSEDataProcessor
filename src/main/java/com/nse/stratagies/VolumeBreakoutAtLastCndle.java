...
public class VolumeBreakoutAtLastCndle {
    ...
    private static void process(int month) {
        ...
        previousMonthData.forEach((s, firstMonthStatistics) -> {
            ...
            if(firstMonthStatistics.getMaxVolumeDate().equals(firstMonthStatistics.getMonthCloseDate().minusDays(1))) {
                firstMonthStatistics.getAction().addAll(findHighAndLows(bhavData, bhavData.get(0)));
            }
        });
