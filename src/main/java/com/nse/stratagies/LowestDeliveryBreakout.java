...
// Wait for delivery breakout should be happen, then find the least delivery volume before the d-volume breakout,
// After volume break out wait for least d-volume should come be. then watch

//  ex : if previous month d-v is 10k
/// on current month 11k dv broken, 
/// from month first to d-v broken find the least delivery(2k) 
/// Wait for less than 2k volume come.
public class LowestDeliveryBreakout {
    ...
    private static void process(int month) {
        ...
        previousMonthData.forEach((s, firstMonthStatistics) -> {
            ...
            BhavData previousCandle = null;
            for (BhavData data : bhavData) {
                if (data.getTotalTradedQty() < firstMonthStatistics.getMinVolumeQty()) {
                    firstMonthStatistics.getAction().add(data.getTradingDate().toString());
                    String candleInfo = getCandleInfo(previousCandle, data);
                    firstMonthStatistics.getAction().add(getCandleInfo(previousCandle, data));
                    firstMonthStatistics.getAction().addAll(findHighAndLows(bhavData, data));
                    results.add(backTest(bhavData, previousCandle, data, candleInfo));
                    break;
                }
                previousCandle = data;
            }
        });
        String reportFileName = String.format(NSEConstant.OPTIONS_DATA_REPORT_FOLDER, fromPreviousMonthStartDate + " to " + toDate + "-DELIVERY.csv");
        FileUtils.saveBackTestData(reportFileName, results);
    }


    public static String getCandleInfo(BhavData previousCandle, BhavData currentCandle) {
        String result = new String();
        if (currentCandle.getLastPrice() > currentCandle.getOpenPrice()) {
            result = result.concat("GREEN-");
        } else if (currentCandle.getLastPrice() < currentCandle.getOpenPrice()) {
        ...
        if (currentCandle.getLastPrice() > previousCandle.getHighPrice()) {
            result = result.concat("SUPPORT");
        } else if (currentCandle.getLastPrice() < previousCandle.getLowPrice()) {
            result = result.concat("RESISTANCE");
        } else if (currentCandle.getHighPrice() > previousCandle.getHighPrice() && currentCandle.getLowPrice() < previousCandle.getLowPrice()) {
            result = result.concat("HIGH-VOLATILE");
        } else if (currentCandle.getHighPrice() < previousCandle.getHighPrice() && currentCandle.getLowPrice() > previousCandle.getLowPrice()) {
            result = result.concat("LOW-VOLATILE");
        } else if (currentCandle.getHighPrice() > previousCandle.getHighPrice()) {
            result = result.concat("RESISTANCE-OPEN");
        } else if (currentCandle.getLowPrice() < previousCandle.getLowPrice()) {
            result = result.concat("SUPPORT-OPEN");
        }
        return result;
    }
