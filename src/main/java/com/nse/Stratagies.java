...
public class Stratagies {
    static final Logger LOGGER = LoggerFactory.getLogger(Stratagies.class);
    ...
    public Stratagies(String from, String to) {
        this.fromDate = from;
        this.toDate = to;
    ...
    public static void main(String[] args) {
        ...
        String to = "30122021";
        Stratagies Stratagies = new Stratagies(from, to);
        List<OptionsData> print = new ArrayList<>();
        Stratagies.maxOIGropByDateAndOptionType.values().stream()
                ...
                .forEach(optionsData -> {
                    ...
                    if (Stratagies.isChangeOIAligned(o1,o2 )) {
                    ...
                    maxOIMap.values().forEach(opd -> {
                        if (o1.isOIAligned() && o2.isOIAligned()) {
                            opd.setOIAligned(true);

                            if (opd.getOptionType().equalsIgnoreCase("PE")) {
                                if (Stratagies.isPriceAboveSupport(opd)) {
                                    opd.setCanTrade(true);
                                }
                            } else if (opd.getOptionType().equalsIgnoreCase("CE")) {
                                if (Stratagies.isPriceBelowResistance(opd)) {
                                    opd.setCanTrade(true);
                                }
                            }

                            if (opd.isCanTrade()) {
                                if (opd.getSymbol().equalsIgnoreCase("BAJAJFINSV")) {
                                    opd.getSymbol();
                                }
                                if (Stratagies.isLowBrokenOnNextDay(opd)) {
                                    OptionsData nextDayOptionsData = Stratagies.getNextDayOptionsData(opd);
                                    ...
                                    opd.setOptionHigh(Stratagies.searchMaxOptionsPrice(nextDayOptionsData).getHigh());
                                    opd.setOptionLow(Stratagies.searchMinOptionsPrice(nextDayOptionsData).getLow());
                                    ...
                                    opd.setResult(Stratagies.getResult(copy));
                                    //}
                                }/* else {
        ...
        //FileUtils.saveDataListToFile(String.format(NSEConstant.OPTIONS_DATA_REPORT_FOLDER, from + "-" + to + "-TRACKING.csv"), Stratagies.trackingResults);
    }
